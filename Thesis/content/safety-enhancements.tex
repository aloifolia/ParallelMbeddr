...
\end{ccode}
Although the previous code would not produce any synchronization gap ParallelMbeddr does not recognize this since the expression \CODE{e.get} of \CODE{.set} does not refer to a named ressource or a variable with a synchronized shared ressource. Instead for exactly this purpose named ressources were implemented which allow to rewrite the code in the following valid manner:
\begin{ccode}
shared<shared<int32>> v;
sync(v) {
  sync(&(v.get) as w) {
