...
For each arity of synchronization ressources seperate versions of the \CODE{start}- and \CODE{stopSyncForNMutexes} functions are declared inside a generic C module. The latter kinds straightforwardly redirect their mutex parameters to calls of the \CODE{pthread\_mutex\_unlock} function:
\begin{ccode}
...
\TODO{sync => start/stop Mutex (start zu Beginn, stop am Ende und evtl. vor Kontrollfluss brechenden statements)}