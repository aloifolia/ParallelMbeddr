...
\end{ccode}
Due to the merging the alias set of \CODE{myV1} becomes $\{$\CODE{p2}$\}$ and is dominated by the alias set $\{$\CODE{p1}, \CODE{p2}$\}$ of \CODE{myV2}. Hence, in the recursive lock algorithm the synchronization resource \CODE{myV2} would removed. The optimized code would therefore become:
\begin{ccode}
...
void bar(shared<int32>* v1, shared<int32>* v2) { 
...
\ForAll{$e.set(\_)$ and $e.get$ in AST of $t$ or AST of either function of \Call{Call Branching}{$t$}}
