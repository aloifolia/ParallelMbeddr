...
public class checkSharedGetSetForSyncContext_NonTypesystemRule extends AbstractNonTypesystemRule_Runtime implements NonTypesystemRule_Runtime {
  ...
  public void applyRule(final SNode sharedGetOrSet, final TypeCheckingContext typeCheckingContext, IsApplicableStatus status) {
    ...
    if (SNodeOperations.isInstanceOf(sharedGetOrSet, "parallelization.structure.SharedGet") || SNodeOperations.isInstanceOf(sharedGetOrSet, "parallelization.structure.SharedSet")) {
      ...
      SNode possibleTaskContext = SNodeOperations.getAncestor(sharedExpr, "parallelization.structure.Task", false, false);

      if (SNodeOperations.isInstanceOf(sharedExpr, "parallelization.structure.SyncRessourceRef")) {
        if ((possibleTaskContext != null) && !(ListSequence.fromList(SNodeOperations.getAncestors(SLinkOperations.getTarget(SNodeOperations.cast(sharedExpr, "parallelization.structure.SyncRessourceRef"), "syncRessource", false), null, false)).contains(possibleTaskContext))) {
          {
            MessageTarget errorTarget = new NodeMessageTarget();
            IErrorReporter _reporter_2309309498 = typeCheckingContext.reportTypeError(sharedGetOrSet, "referenced sync ressource not synchronized in surrounding task context", "r:daf934de-3466-4fa8-a227-270fedb7e2f2(parallelization.typesystem)", "4335879941194289101", null, errorTarget);
          }
        }
        ...
        for (SNode sync : Sequence.fromIterable(SNodeOperations.ofConcept(SNodeOperations.getAncestors(sharedExpr, null, false), "parallelization.structure.SyncStatement"))) {
        ...
        if ((matchedSyncRessource == null)) {
          {
            MessageTarget errorTarget = new NodeMessageTarget();
            IErrorReporter _reporter_2309309498 = typeCheckingContext.reportTypeError(sharedGetOrSet, "referred variable is not synchronized", "r:daf934de-3466-4fa8-a227-270fedb7e2f2(parallelization.typesystem)", "4335879941194458996", null, errorTarget);
          }
          ...
          {
            MessageTarget errorTarget = new NodeMessageTarget();
            IErrorReporter _reporter_2309309498 = typeCheckingContext.reportTypeError(sharedGetOrSet, "referred variable is not synchronized in surrounding task context", "r:daf934de-3466-4fa8-a227-270fedb7e2f2(parallelization.typesystem)", "4335879941194483198", null, errorTarget);
          }
        }
      } else {
        {
          MessageTarget errorTarget = new NodeMessageTarget();
          IErrorReporter _reporter_2309309498 = typeCheckingContext.reportTypeError(sharedGetOrSet, "expression must be synchronized", "r:daf934de-3466-4fa8-a227-270fedb7e2f2(parallelization.typesystem)", "5853110027233019383", null, errorTarget);
        }
      }
    }
  }
