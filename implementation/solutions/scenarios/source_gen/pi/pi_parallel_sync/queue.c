...
=> native message passing might also be helpful (especially for directed communication
   ...
   provides real benefit for the communication) */
      ...
      {
        GenericSyncDeclarations_stopSyncFor1Mutex(&queue->mutex);
  struct timespec sleepingTime = (struct timespec){ .tv_nsec = 20};
...
inline int32_t queue_queueGetInsertAt(queue_SharedTypes_0_SharedOf_Queue_0_t* queue) 
{
  //GenericSyncDeclarations_startSyncFor1Mutex(&queue->mutex);
  {
    //GenericSyncDeclarations_stopSyncFor1Mutex(&queue->mutex);
...
inline void queue_queueSetInsertAt(queue_SharedTypes_0_SharedOf_Queue_0_t* queue, int32_t newInsertAt) 
{
  //GenericSyncDeclarations_startSyncFor1Mutex(&queue->mutex);
  {
...
inline int32_t queue_queueGetDeleteAt(queue_SharedTypes_0_SharedOf_Queue_0_t* queue) 
{
  //GenericSyncDeclarations_startSyncFor1Mutex(&queue->mutex);
  {
    //GenericSyncDeclarations_stopSyncFor1Mutex(&queue->mutex);
...
inline void queue_queueSetDeleteAt(queue_SharedTypes_0_SharedOf_Queue_0_t* queue, int32_t newDeleteAt) 
{
  //GenericSyncDeclarations_startSyncFor1Mutex(&queue->mutex);
  {
...
inline long double queue_queueGetItemAt(queue_SharedTypes_0_SharedOf_Queue_0_t* queue, int32_t index) 
{
  //GenericSyncDeclarations_startSyncFor1Mutex(&queue->mutex);
  {
    {
...
inline void queue_queueSetItemAt(queue_SharedTypes_0_SharedOf_Queue_0_t* queue, int32_t index, long double newItem) 
{
  //GenericSyncDeclarations_startSyncFor1Mutex(&queue->mutex);
  {
    {
